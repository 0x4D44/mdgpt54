...
## Live aircraft (airplanes.live)

Live aircraft come from **airplanes.live**, fetched browser-direct: it serves
`Access-Control-Allow-Origin: *`, so no proxy/server is needed and it works on GitHub Pages and
`localhost` alike. The client queries `GET /v2/point/{lat}/{lon}/{radiusNm}` (viewport centre +
radius, capped at 250 nm) and maps the response in `src/traffic/airplanesLive.ts`.

OpenSky was the original source but is no longer usable browser-side: it returns
`Access-Control-Allow-Origin: https://opensky-network.org` (CORS-blocks every other origin) and its
anonymous API rate-limits per IP, so shared datacenter IPs (any serverless proxy) are dropped with
HTTP 522 — authentication doesn't help because the connection itself times out. OpenSky is still used
for the static aircraft-identity metadata shards (by `icao24`). The click-to-show flight-route arc used
OpenSky's `/routes` endpoint and is therefore unavailable (it degrades silently to no arc).

Personal, non-commercial use; please keep within airplanes.live's usage policy (≤ ~1 req/s — the app
polls every 15 s) and retain attribution.
...
- airplanes.live for live aircraft (browser-direct, permissive CORS)
...
- This build targets GitHub Pages: the static globe, live aircraft (airplanes.live), and aircraft identity all work browser-direct with no server.
